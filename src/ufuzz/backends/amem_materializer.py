"""Certified materialization for the pinned A-Mem native note profile.

The upstream ingestion path remains native: root replay invokes ``add_note``
and is published only when its complete public MemoryNote projection and
...
AMEM_NATIVE_CONFIGURATION_ID = f"a-mem-native:{AMEM_COMMIT}:memory-note:chroma:v1"
...
class AMemMaterializationProtocol(MaterializationProtocol[StateHandle]):
    """Materialize and certify one dedicated-process A-Mem state."""

    backend = "a-mem"
    frozen_configuration_id = AMEM_NATIVE_CONFIGURATION_ID
    ...
    def __init__(
        ...
        *,
        ...
        adapter: AMemAdapter | None = None,
    ) -> None:
        if not initialization_artifacts or not frozen_inventories:
            raise ValueError("A-Mem materializer requires artifact and inventory registries")
        for key, artifact in initialization_artifacts.items():
            ...
            if artifact.frozen_config.get("amem_profile_id") != self.frozen_configuration_id:
                raise ValueError("artifact does not freeze the selected A-Mem profile")
        ...
        self._adapter = adapter or AMemAdapter()
    ...
    def _validate_root(self, request: RootMaterializationRequest):
        if (request.backend, request.frozen_configuration_id) != (
            ...
            raise _EvidenceError("root request does not use the frozen A-Mem profile")
    ...
    def _root_rebinding(self, request, frozen, fresh, artifact):
        ...
        if any(len(values) != 1 for values in (*old.values(), *new.values())):
            raise _AmbiguousRebinding("duplicate exact A-Mem replay key is ambiguous")
        bindings = tuple(
            sorted(
                (
                    ReplayBinding(
    ...
    def _context(self, state):
        ...
        if value is None or value.handle is not state.handle:
            raise _EvidenceError("unknown or retired A-Mem materialized state")
        if (
            state.backend,
        ...
        transition = _digest(
            {
                ...
                "auxiliary": "Chroma document is derived from each MemoryNote public row",
            }
        )
        return DescendantStateCertificate(
            "a-mem:descendant:" + _digest((observable, transition)),
            self.backend,
            self.frozen_configuration_id,
            state.campaign_id,
            state.root_checkpoint_id,
    ...
    async def materialize_root(self, request):
        ...
        try:
            ...
            capability = AMemRetrievableEntryCapability(handle)
            ...
            return self._failure(
                ...
                f"A-Mem root creation failed: {type(error).__name__}: {error}",
            )
    ...
    def _parse(artifact):
        ...
        if name is None or payload.get("operation") != name:
            raise _EvidenceError("A-Mem materializer accepts only exact memory mutations")
        if not {"operation", "new_text", "provenance_ids"}.issubset(payload) or not set(
            payload
        ).issubset({"operation", "new_text", "provenance_ids", "context"}):
    ...
    def _resolve_target(self, context, public, artifact):
        ...
        if len(matches) != 1:
            raise _EvidenceError("target lineage does not resolve to exactly one live note")
    ...
    def _receipt(receipt: OperationReceipt, state_id: str, operation: str, target: str):
        expected = "unrelated_existing_region_change" if operation == "unrelated_change" else operation
        return (
            ...
            and receipt.affected_local_ids == (target,)
        )
        ...
        try:
            ...
            if operation.name == "delete":
                ...
                if any(before[key] != after[key] for key in after):
                    raise _EvidenceError("delete changed an unrelated public note")
                successors = ()
                kind = AffectedTransitionKind.DELETED
                outcome = PhysicalTransitionOutcome.DELETED
            else:
                if set(before) != set(after):
                    raise _EvidenceError("update added/deleted a public note")
                ...
                if observable["content"] != operation.new_text:
                    raise _EvidenceError("updated note content differs from exact payload")
                if after[identifier].provenance_ids != before[identifier].provenance_ids:
                    raise _EvidenceError("updated note lost or changed source provenance")
            ...
            closure = await context.capability.retrieve(
                ...
                root_checkpoint_id=state.root_checkpoint_id,
                state_id=state.state_id,
                query=target.projection.observable["content"],
            ...
            transition_id = f"a-mem:target:{artifact.artifact_id}"
            proof = _digest(
                {
                    ...
                    "post": None
                    if not successors
                    else after[identifier].projection.observable,
                }
            )
            ...
            certificate = PhysicalTransitionCertificate(
                f"a-mem:transition:{artifact.artifact_id}",
            ...
            return self._failure(
                ...
                state.root_checkpoint_id,
                MaterializationFailureKind.TRANSIENT_BACKEND_FAILURE,
                f"A-Mem transition failed: {type(error).__name__}: {error}",
            )
