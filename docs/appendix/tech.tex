...
Coverage-Guided also starts with \(\mathrm{seen}=\varnothing\). For execution
\(t\), it uses only
\[
\mathrm{score}_t=
|\operatorname{CoverageIDs}(R_t)\setminus\mathrm{seen}|,
\qquad
\mathrm{seen}\leftarrow\mathrm{seen}\cup
\operatorname{CoverageIDs}(R_t).
\]
Initialization retrievals receive no credit. Full \system replaces its former
canonical-region coverage component with this observed new-entry concept while
retaining separate retrieval-pattern novelty and meaning-preserving ranked
