...
\paragraph{Coverage feedback.}
Coverage-Guided starts with an empty \(\mathrm{seen}\) set and scores execution
\(t\) by
\begin{equation}
...
\label{eq:coverage-guided-score}
\end{equation}
It uses no other feedback. \system uses the same observed new-entry concept in
place of canonical-region gain, while retaining its separate retrieval-pattern
novelty and meaning-preserving ranked-divergence components. No search method
receives evaluator-only information.
...
version, normalization algorithm, and manual-audit sample size are selected
after the pilot and frozen before full evaluation.
